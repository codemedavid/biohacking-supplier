    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-white">
                ...
                <div className="max-w-4xl mx-auto px-3 sm:px-4">
                    <div className="flex items-center justify-between h-12 md:h-14">
                        <div className="flex items-center space-x-2">
                            ...
                            <h1 className="text-sm md:text-base font-bold text-charcoal-800 flex items-center gap-2">
            ...
            <div className="max-w-4xl mx-auto px-3 sm:px-4 py-4">
                ...
                {/* Add New Form */}
                {isAdding && (
                    <div className="mb-4 bg-white backdrop-blur-md border border-charcoal-200 rounded-xl p-4 shadow-sm">
                        <h3 className="font-semibold text-charcoal-800 mb-3">Add New Shipping Location</h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div>
                                <label className="block text-xs font-medium text-charcoal-700 mb-1">ID (e.g., CEBU)</label>
                                <input
                                    type="text"
                                    value={newLocation.id}
                                    onChange={(e) => setNewLocation({ ...newLocation, id: e.target.value.toUpperCase() })}
                                    className="w-full px-3 py-2 border border-charcoal-600/50 rounded-lg text-sm text-charcoal-800"
                                    placeholder="LOCATION_ID"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-charcoal-700 mb-1">Display Name</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs font-medium text-charcoal-700 mb-1">Fee (₱)</label>
                                <input
                                    type="number"
                        ...
                        <div className="flex gap-2 mt-3">
                            ...
                            <button
                                ...
                                className="bg-charcoal-800/50 text-charcoal-700 px-4 py-2 rounded-lg text-sm font-medium"
                            >
                                Cancel
                            </button>
                        </div>
                ...
                <div className="space-y-3">
                    {locations.map((location) => (
                        <div
                            key={location.id}
                            className={`bg-white backdrop-blur-md border rounded-xl p-4 shadow-sm transition-all ${location.is_active ? 'border-charcoal-200' : 'border-red-200 bg-red-50/30'
                                }`}
                        >
                            {editingId === location.id ? (
                                <div className="flex flex-col md:flex-row gap-3">
                                    <div className="flex-1">
                                        <label className="block text-xs font-medium text-charcoal-700 mb-1">Name</label>
                                        <input
                                            type="text"
                                            value={editName}
                                    ...
                                    <div className="flex items-end gap-2">
                                        ...
                                        <button
                                            ...
                                            className="p-2 bg-gray-200 text-charcoal-700 rounded-lg hover:bg-gray-300"
                                        >
                                            <X className="w-4 h-4" />
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs px-2 py-0.5 rounded bg-charcoal-800/50 text-charcoal-600 font-mono">
                                                    {location.id}
                                                </span>
                                                {!location.is_active && (
                                            ...
                                            <p className="font-medium text-charcoal-800 mt-1">{location.name}</p>
                                        </div>
                                    </div>
                ...
                {/* Info */}
                <div className="mt-6 bg-theme-bg border border-charcoal-200 rounded-xl p-4">
                    <h4 className="font-medium text-charcoal-800 mb-2">ℹ️ How it works</h4>
