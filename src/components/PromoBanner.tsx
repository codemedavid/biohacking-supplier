...
const PromoBanner: React.FC = () => {
    ...
    return (
        <div className="w-full bg-gradient-to-r from-blush-50 via-white to-glow-teal-50/80 border-b border-charcoal-100">
            <div className="container mx-auto px-4 py-2.5 flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 relative">
                ...
                {submitted ? (
                    ...
                    <>
                        <div className="flex items-center gap-2 text-sm text-charcoal-600 pr-6 sm:pr-0">
                            <Sparkles className="w-4 h-4 text-glow-teal-400 flex-shrink-0" />
                        ...
                        <form onSubmit={handleSubmit} className="flex items-center gap-2 w-full sm:w-auto pr-6 sm:pr-0">
                            <div className="relative flex-1 sm:flex-none">
                                <Mail className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-charcoal-400" />
                                <input
                                    type="email"
                                    value={email}
                                    onChange={(e) => setEmail(e.target.value)}
                                    placeholder="you@gmail.com"
                                    className="pl-8 pr-3 py-1.5 text-sm bg-white border border-charcoal-200 rounded-xl text-charcoal-700 placeholder-charcoal-400 focus:ring-1 focus:ring-glow-teal-300 focus:border-glow-teal-300 outline-none transition-all w-full sm:w-52"
                                    required
                                />
                            </div>
                            <button
                                type="submit"
                                className="px-4 py-1.5 text-sm font-bold bg-gradient-to-r from-glow-teal-400 to-glow-teal-500 hover:from-glow-teal-500 hover:to-glow-teal-600 text-white rounded-xl transition-all whitespace-nowrap shadow-sm"
                            >
                                Subscribe
                            </button>
                        </form>
