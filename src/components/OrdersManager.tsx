      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 text-xs md:text-sm">
            ...
            <div>
              <span className="text-charcoal-400 text-[10px] md:text-xs">Total</span>
              <p className="font-semibold text-gold-600">${finalTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
              {order.shipping_fee && order.shipping_fee > 0 && (
                <p className="text-[10px] md:text-xs text-charcoal-400">+ ${order.shipping_fee} shipping</p>
              )}
            </div>
            <div>
        ...
        <div className="bg-white backdrop-blur-md rounded-lg md:rounded-xl shadow-lg p-4 md:p-6 border border-charcoal-200 space-y-4 md:space-y-6">
          ...
          {order.payment_proof_url && (
            <div>
              ...
              <div className="bg-theme-bg rounded-lg p-3 md:p-4">
                <img
                  src={order.payment_proof_url}
                  alt="Payment proof"
                  className="max-w-full h-auto rounded-lg border border-charcoal-600/50"
                  onError={(e) => {
                    e.currentTarget.style.display = 'none';
                    e.currentTarget.parentElement!.innerHTML = `
                      <div class="text-red-600 p-3 md:p-4 text-center text-xs md:text-sm">
                        <p>⚠️ Payment proof image failed to load</p>
                        <p class="text-[10px] md:text-xs text-charcoal-400 mt-2">URL: ${order.payment_proof_url}</p>
                      </div>
                    `;
                  }}
                />
          ...
          <div className="border-t-2 border-charcoal-200 pt-3 md:pt-4 text-charcoal-800">
            <div className="space-y-1.5 md:space-y-2 text-xs md:text-sm">
              <div className="flex justify-between">
                <span>Subtotal:</span>
                <span className="font-semibold">${(order.total_price + (order.discount_applied || 0)).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
              </div>
              {order.discount_applied && order.discount_applied > 0 && (
                <div className="flex justify-between text-green-600">
                  <span>Discount ({order.promo_code || 'PROMO'}):</span>
                  <span className="font-semibold">-${order.discount_applied.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                </div>
              )}
              {order.shipping_fee && order.shipping_fee > 0 && (
                <div className="flex justify-between">
