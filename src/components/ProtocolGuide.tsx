        ...
        <div className="min-h-screen bg-theme-bg">
            ...
            <main className="container mx-auto px-4 py-8 max-w-4xl">
                ...
                <button
                    ...
                    className="flex items-center gap-2 text-charcoal-300 hover:text-rose-500 transition-colors mb-6 group"
                >
                ...
                <div className="text-center mb-10">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-charcoal-900/80 backdrop-blur-sm border border-glow-teal-500/30 shadow-soft mb-4">
                        <BookOpen className="w-4 h-4 text-rose-500" />
                        <span className="text-xs font-medium text-charcoal-200 uppercase tracking-widest">Protocol Guide</span>
                    </div>
                ...
                {/* General Guidelines */}
                <div className="bg-charcoal-900/40 backdrop-blur-md rounded-2xl shadow-soft border border-charcoal-700/50 p-6 mb-8">
                ...
                <div className="bg-charcoal-900/40 backdrop-blur-md rounded-2xl shadow-soft border border-charcoal-700/50 p-6 mb-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div className="bg-charcoal-800/60 rounded-xl p-4">
                            <p className="font-semibold text-charcoal-200 mb-1">Lyophilized (Powder)</p>
                            <p className="text-charcoal-300">Store at -20°C for long-term. Stable at 2-8°C for weeks.</p>
                        </div>
                        <div className="bg-charcoal-800/60 rounded-xl p-4">
                            <p className="font-semibold text-charcoal-200 mb-1">Reconstituted</p>
                            <p className="text-charcoal-300">Refrigerate at 2-8°C. Use within 14-28 days depending on peptide.</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-6">
                    <label className="block text-sm font-medium text-charcoal-300 mb-2">Filter by Category</label>
                    <select
                        value={selectedCategory}
                        onChange={(e) => setSelectedCategory(e.target.value)}
                        className="w-full sm:w-64 px-4 py-3 rounded-xl bg-charcoal-900/40 backdrop-blur-md border border-glow-teal-500/30 text-charcoal-200 font-medium shadow-soft focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all cursor-pointer"
                    >
                ...
                {loading ? (
                    ...
                    <div className="bg-charcoal-900/40 backdrop-blur-md rounded-2xl shadow-soft border border-charcoal-700/50 p-8 text-center">
                    ...
                    <div className="space-y-4">
                        {filteredProtocols.map((protocol) => (
                            <div
                                key={protocol.id}
                                className="bg-charcoal-900/40 backdrop-blur-md rounded-2xl shadow-soft border border-charcoal-700/50 overflow-hidden"
                            >
                                <button
                                    onClick={() => toggleProtocol(protocol.id)}
                                    className="w-full p-5 flex items-center justify-between text-left hover:bg-charcoal-800/60/50 transition-colors"
                                >
                                    <div>
                                        <span className="text-xs font-medium text-rose-500 uppercase tracking-wider">{protocol.category}</span>
                                        <h3 className="font-heading text-lg font-semibold text-white mt-1">{protocol.name}</h3>
                                    </div>
                                ...
                                {expandedProtocol === protocol.id && (
                                    <div className="px-5 pb-5 border-t border-charcoal-700/50">
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 mb-4">
                                            <div className="bg-charcoal-800/60 rounded-xl p-3">
                                                <p className="text-xs text-charcoal-500 uppercase tracking-wider">Dosage</p>
                                                <p className="text-sm font-semibold text-charcoal-200 mt-1">{protocol.dosage}</p>
                                            </div>
                                            <div className="bg-charcoal-800/60 rounded-xl p-3">
                                                <p className="text-xs text-charcoal-500 uppercase tracking-wider flex items-center gap-1">
                                                    <Clock className="w-3 h-3" /> Frequency
                                                </p>
                                                <p className="text-sm font-semibold text-charcoal-200 mt-1">{protocol.frequency}</p>
                                            </div>
                                            <div className="bg-charcoal-800/60 rounded-xl p-3">
                                                <p className="text-xs text-charcoal-500 uppercase tracking-wider">Duration</p>
                                                <p className="text-sm font-semibold text-charcoal-200 mt-1">{protocol.duration}</p>
                                            </div>
                                        </div>
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <ul className="space-y-2">
                                                    {protocol.notes.map((note, idx) => (
                                                        <li key={idx} className="flex items-start gap-2 text-sm text-charcoal-200">
                                                            <span className="w-1.5 h-1.5 rounded-full bg-rose-400 mt-2 flex-shrink-0"></span>
                                                            {note}
                                                        </li>
                                                    ))}
